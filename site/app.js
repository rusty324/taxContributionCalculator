...
function readInput() {
  ...
  return {
    ...
    famliBenefits: num(fd.famliBenefits),
    famliFedWithheld: num(fd.famliFedWithheld),
...
function render(input, r) {
  ...
  const rows = [
    ...
    r.famliWithheld > 0 ? row('Withheld from FAMLI benefits', money(r.famliWithheld)) : '',
...
function renderColorado(fed, r) {
  ...
  const rows = [
    ...
    L.famliSubtraction > 0 ? row('− FAMLI benefits', money(L.famliSubtraction)) : '',
    L.otherSubtractions > 0 ? row('− Other subtractions', money(L.otherSubtractions)) : '',
